...
def update_packing_google_sheets(csv_file_path):  
    try:  
        ...
        # ✅ CORREÇÃO FINAL: leitura robusta para CSVs mal formatados  
        df = pd.read_csv(  
            csv_file_path,  
            encoding='latin1',  
            engine='python',           # parser Python (mais tolerante)  
            on_bad_lines='skip',       # pula linhas com formato inválido  
            skipinitialspace=True,     # ignora espaços após vírgulas  
            na_filter=False            # evita que campos vazios sejam tratados como NaN  
        ).fillna("")  
          
        # ✅ Verificação básica: se o DataFrame estiver vazio, avisa  
        if df.empty:  
            print("⚠️ O CSV está vazio após o processamento. Verifique o arquivo.")  
            return  
  
        # Limpa a aba e atualiza  
        worksheet1.clear()  
